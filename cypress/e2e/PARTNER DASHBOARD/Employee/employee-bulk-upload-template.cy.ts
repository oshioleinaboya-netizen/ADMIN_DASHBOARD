...
describe('Employee upload', () => {
    ...
    it('Employee upload - Check for file upload functionality', () => {
        ...
        cy.contains(uploadOptions[1]).click()
        cy.contains('Continue').should('not.be.disabled').click() // Click on continue button after selecting upload option

        cy.url().should('include', '/employee/bulk-upload') // Check if URL includes /employee/upload
        ...
        cy.wait(3000)
        //
        cy.url().should('include', 'employee/bulk-upload') // Upload preview page check
